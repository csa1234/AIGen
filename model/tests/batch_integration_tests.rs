...
async fn full_workflow_updates_job_status() {
    ...
    manager
        .subscribe_user("0x00000000000000000000000000000000000001aa", SubscriptionTier::Pro, false, now)
        .expect("subscribe");

    let input = sample_input();
    let payload = sample_payload("0x00000000000000000000000000000000000001aa", BatchPriority::Standard, "model-a", input.clone());
    let tx = make_tx("0x00000000000000000000000000000000000001aa", CEO_WALLET, 10, payload);
    let job_id = queue
        .validate_and_submit(&tx, "model-a".to_string(), input.clone())
        .await
        .expect("submit");

    queue.reschedule_job(&job_id, model::now_timestamp()).await.expect("reschedule");
    let job = queue.get_next_ready_job().await.expect("ready").expect("job");
    queue
        .update_job_status(&job.request_id, BatchJobStatus::Processing, None, None)
        .await
        .expect("processing");
    queue
        .update_job_status(&job.request_id, BatchJobStatus::Completed, Some(vec![9]), None)
        .await
        .expect("completed");
...
async fn multiple_priorities_follow_order() {
    ...
    let now = model::now_timestamp();
    manager
        .subscribe_user("0x00000000000000000000000000000000000001bb", SubscriptionTier::Pro, false, now)
        .expect("subscribe");

    let input = sample_input();
    let standard_payload = sample_payload("0x00000000000000000000000000000000000001bb", BatchPriority::Standard, "model-a", input.clone());
    let batch_payload = sample_payload("0x00000000000000000000000000000000000001bb", BatchPriority::Batch, "model-a", input.clone());
    let economy_payload = sample_payload("0x00000000000000000000000000000000000001bb", BatchPriority::Economy, "model-a", input.clone());

    let standard_tx = make_tx("0x00000000000000000000000000000000000001bb", CEO_WALLET, 10, standard_payload);
    let batch_tx = make_tx("0x00000000000000000000000000000000000001bb", CEO_WALLET, 5, batch_payload);
    let economy_tx = make_tx("0x00000000000000000000000000000000000001bb", CEO_WALLET, 3, economy_payload);
...
async fn volume_discounts_lower_expected_payment() {
    ...
    let now = model::now_timestamp();
    manager
        .subscribe_user("0x00000000000000000000000000000000000001cc", SubscriptionTier::Unlimited, false, now)
        .expect("subscribe");
    ...
    let input = sample_input();
    let payload = sample_payload("0x00000000000000000000000000000000000001cc", BatchPriority::Standard, "model-a", input.clone());
    let tx = make_tx("0x00000000000000000000000000000000000001cc", CEO_WALLET, 9, payload);
...
async fn tier_enforcement_blocks_free_allows_pro() {
    ...
    let now = model::now_timestamp();
    manager
        .subscribe_user("0x00000000000000000000000000000000000001dd", SubscriptionTier::Free, false, now)
        .expect("subscribe");
    manager
        .subscribe_user("0x00000000000000000000000000000000000001ee", SubscriptionTier::Pro, false, now)
        .expect("subscribe");

    let input = sample_input();
    let free_payload = sample_payload("0x00000000000000000000000000000000000001dd", BatchPriority::Standard, "model-a", input.clone());
    let free_tx = make_tx("0x00000000000000000000000000000000000001dd", CEO_WALLET, 10, free_payload);
    let free_res = queue.validate_and_submit(&free_tx, "model-a".to_string(), input.clone()).await;
    assert!(matches!(free_res, Err(BatchError::InsufficientTier)));

    let pro_payload = sample_payload("0x00000000000000000000000000000000000001ee", BatchPriority::Standard, "model-a", input.clone());
    let pro_tx = make_tx("0x00000000000000000000000000000000000001ee", CEO_WALLET, 10, pro_payload);
    let pro_res = queue.validate_and_submit(&pro_tx, "model-a".to_string(), input).await;

...
//
// Trust Wallet wrapper

class TrustProvider extends (window.AigenBaseProvider || class {}) {
  constructor() {
    super();
    this.name = 'Trust Wallet';
    this.ethereum = typeof window !== 'undefined' ? window.ethereum : null;
  }
  isAvailable() { return !!(this.ethereum && this.ethereum.isTrust); }
  async connect() {
    if (!this.isAvailable()) throw new Error('Trust Wallet not available');
...
if (typeof window !== 'undefined') window.AigenTrustProvider = TrustProvider;
if (typeof module !== 'undefined' && module.exports) module.exports = TrustProvider;

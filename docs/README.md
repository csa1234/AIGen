# AIGEN Documentation

## Getting Started
- **[Complete Guide](../GUIDE.md)** - Comprehensive setup and usage guide
- **[README](../README.md)** - Project overview and quick start

## Specialized Guides
- **[API Reference](API.md)** - Complete RPC API documentation
- **[Production Deployment](PRODUCTION_GUIDE.md)** - Production deployment guide
- **[Testing Guide](TESTING.md)** - Testing and validation
- **[Genesis Key Security](GENESIS_KEY_SECURITY.md)** - Security model and key management
- **[Economic Tiers](ECONOMIC_TIERS.md)** - Subscription tiers and pricing
- **[AI Models](AI_MODELS.md)** - AI model management
- **[Phased Roadmap](roadmap.md)** — 4-phase implementation plan with timelines, milestones, and team owners

## Applications
- **[Admin Dashboard](../dashboard/README.md)** - Web-based admin interface
- **[Playground](playground/README.md)** - Interactive AI playground

## Code Examples
- **[JavaScript Examples](examples/javascript/README.md)** - JavaScript SDK examples
- **[Python Examples](examples/python/README.md)** - Python SDK examples
- **[cURL Examples](examples/curl/README.md)** - Raw API examples
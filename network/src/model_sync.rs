...
impl ModelSyncManager {
    ...
    pub fn handle_model_announcement(&self, model_id: String, shard_index: u32, peer: PeerId) {
        ...
        let _ = self.event_tx.try_send(NetworkEvent::ModelShardAnnounced {
            model_id,
            shard_index,
            peer: Some(peer),
        });
    ...
    pub async fn process_download_queue(&self) -> Result<(), ModelSyncError> {
        ...
        while self.active_downloads.len() < MAX_CONCURRENT_DOWNLOADS {
            ...
            let peer = self
                ...
                .ok_or_else(|| {
                    ModelSyncError::NoAvailablePeers(task.model_id.clone(), task.shard_index)
                })?;
    ...
    pub async fn handle_received_shard(
        ...
        self.metrics
            .add_model_bytes_received(response.data.len() as u64);
        Ok(())
    }

    pub async fn verify_downloaded_shard(
        &self,
        response: &ModelShardResponse,
    ) -> Result<(), ModelSyncError> {
        ensure_running()?;
    ...
    pub fn handle_download_failure(
        ...
        self.active_downloads
            .remove(&(model_id.to_string(), shard_index));
    ...
    async fn persist_downloaded_shard(
        &self,
        response: &ModelShardResponse,
    ) -> Result<(), ModelSyncError> {
        ...
        tokio::fs::write(&path, &response.data)
            .await
            .map_err(StorageError::from)?;
        let verified = verify_shard_integrity(&path, &shard.hash)
            ...
            .map_err(StorageError::from)?;
        if !verified {
            return Err(ModelSyncError::Storage(
        ...
        let _ = self.registry.register_shard_location(
            &response.model_id,

...
mod chat_completion_tests {
    ...
    use model::{TierManager, InferenceEngine, ModelRegistry, BatchQueue, DefaultPaymentProvider, LocalStorage, VolumeDiscountTracker};
    use crate::rpc::types::*;
    ...
    async fn setup_test_node() -> ModelRpcMethods {
        // Create a simple payment provider for testing
        ...
        let storage = Arc::new(LocalStorage::new(PathBuf::from("./test_models")));
        
        // Create inference engine with minimal config
        ...
        // Create batch queue with required dependencies
        let batch_queue = Arc::new(BatchQueue::new(
            tier_manager.clone(),
            inference_engine.clone(),
            None, // No ad manager for tests
            Arc::new(blockchain_core::ChainState::new()), // Simplified chain state
    ...
    fn create_test_transaction(
        ...
        let tx = Transaction {
            ...
            signature: sign_message(&[], &signing_key), // This would be properly signed in real usage
    ...
    async fn test_chat_completion_with_subscription() {
        ...
        let _tier_manager = rpc.tier_manager.clone();
    ...
    async fn test_chat_completion_free_tier_ad_injection() {
        let _rpc = setup_test_node().await;
